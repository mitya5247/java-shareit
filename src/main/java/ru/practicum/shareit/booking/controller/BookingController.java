...
public class BookingController {
    ...
    @PostMapping
    public BookingDtoResponse createRequest(@RequestHeader(Constants.HEADER) Long userId, @Valid @RequestBody BookingDto bookingDto) throws EntityNotFound, ItemIsUnAvailable, BookingDtoIsNotValid {
    ...
    public BookingDtoResponse updateState(@RequestHeader(Constants.HEADER) Long userId, @PathVariable Long bookingId,
                                          @RequestParam(name = "approved") String state) throws EntityNotFound, UnknownState {
        return service.updateState(userId, bookingId, state);
    }

    @GetMapping(Constants.PATH_BOOKING_ID)
    public BookingDtoResponse get(@RequestHeader(Constants.HEADER) Long userId, @PathVariable Long bookingId) throws EntityNotFound {
    ...
    public List<BookingDtoResponse> getAllUserBookings(@RequestHeader(Constants.HEADER) Long userId,
            ...
            defaultValue = "0") Long from, @RequestParam(required = false, defaultValue = "10") Long size) throws EntityNotFound, UnknownState {
